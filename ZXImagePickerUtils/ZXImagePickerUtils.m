...
@interface ZXImagePickerUtils ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    ZXTakeEndAction action;
}
@end

@implementation ZXImagePickerUtils

-(void)takePhotoUponVC:(UIViewController *)vc
              callBack:(ZXTakeEndAction)callBack{
    action = callBack;
    if ([ZXImagePickerUtils isCameraAvailable] &&
        [ZXImagePickerUtils doesCameraSupportTakingPhotos]) {
        ...
        if ([ZXImagePickerUtils isAuthorized_jumpToSettingifNo:true]) {
            [vc presentViewController:pickerVC animated:true completion:nil];
        }
    }else{
        if (action) {
            action(nil,ZXPCameraDisable,@"该设备不支持拍照");
        }
    }
}
...
- (void)choosePhotoUponVC:(UIViewController *)vc
    ...
    if ([ZXImagePickerUtils isPhotoLibraryAvailable]) {
        ...
        if ([ZXImagePickerUtils canUsePickVideosFromPhotoLibrary]) {
        ...
        if (action) {
            action(nil,ZXPCameraDisable,@"该设备不支持资源选择");
        }
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if ([mediaType isEqualToString:(__bridge NSString *)kUTTypeImage]) {
        ...
        [picker dismissViewControllerAnimated:true completion:^{
            if (action) {
                action(image,ZXPSuccess,nil);
            }
        }];
    }else{
        [picker dismissViewControllerAnimated:true completion:^{
            if (action) {
                action(nil,ZXPNotImage,@"获取的不是图片");
            }
        }];
    }
}
